...
pub(super) fn first_placeholder_index(source: &str, start: usize, end: usize) -> Option<usize> {
    TargetScanner::new(source, start, end, '?').next_index()
}

pub(super) fn placeholder_count(source: &str) -> usize {
    TargetScanner::new(source, 0, source.len(), '?').count()
}
...
impl<'a> TargetScanner<'a> {
    const fn new(source: &'a str, start: usize, end: usize, target: char) -> Self {
        Self {
            source,
            index: start,
            end,
    ...
    fn next_index(&mut self) -> Option<usize> {
        while !self.is_at_end() {
            if self.starts_with("/*") {
                ...
                self.skip_quoted();
            } else if self.current_char() == Some(self.target) {
